// VersionDriftTest.kt
// The version lives in two files that must move together: AppVersion.VERSION (what the app
// reports and the update check compares) and build.gradle.kts's packageVersion (what jpackage
// stamps on the installers). Reads the build script off disk; the test runs with the repo root
// as its working directory. Carried over from PGPonyDesktop 1.1.0.
...
class VersionDriftTest {
    ...
    fun versionIsPlainSemver() {
        // The update check and the downstream winget/AUR templates assume x.y.z with no suffix.
